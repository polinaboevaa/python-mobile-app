...
async def add_schedule(data: ScheduleModel, db: AsyncSession = Depends(get_async_db)):
    ...
    schedule_id = await Repository.add_schedule(db, data)
    return ScheduleIdModel(schedule_id=schedule_id)

@router.get("/schedules") 
async def get_schedules(user_id: int, db: AsyncSession = Depends(get_async_db)):
    """Возвращает список ID расписаний выбранного пользователя"""
    schedules = await ScheduleService.get_schedules_ids(user_id, db)
    return schedules

@router.get("/schedule")
async def get_schedule_for_user_by_id(schedule_id: int, user_id: int, db: AsyncSession = Depends(get_async_db)):
...
async def get_schedule_for_period(user_id: int, db: AsyncSession = Depends(get_async_db)):
